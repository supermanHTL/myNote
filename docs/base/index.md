...
window.addEventListener('message', function (event) {
  if (event.origin == 'http://www.wrox.com') {
    //接收到的数据
    console.log(event.data); //可选：向来源窗口发送回执
...
_以上代码执行后，你就会发现当拖动着元素移动到放置目标上时，光标变成了允许放置的符号。当然，释放鼠标也会触发 `drop` 事件。_
...
- `space` （**选填**）指定缩进用的空白字符串。如果参数是个数字，它代表有多少的空格；上限为 10。该值若小于 1，则意味着没有空格；如果该参数为字符串（当字符串长度超过 10 个字母，取其前 10 个字母），该字符串将被作为空格；如果该参数没有提供（或者为 `null`），将没有空格。
  ...
  **例二：** 数组过滤器
  ...
  ```javascript
  const obj = { name: 'htl', age: 29, like: ['html', 'css', 'js'] };
  const res2 = JSON.stringify(obj, ['name', 'age'], 'abcd');
